...
import * as vs from '../vs/tree_view_provider';
...
export class TreeViewProvider extends absprovider.TreeViewProviderProjects {
  ...
  public ReloadProject(file: string): [model.Project[], string[]] {
    ...
    var xml_object = xml.parseString(content, (err, result) => {
      if (!result.hasOwnProperty('CodeBlocks_project_file') ||
          ...
          !result.CodeBlocks_project_file.Project[0].Build[0].hasOwnProperty(
              'Target')) {
        return;
      }

      var targets: any[] =
          result.CodeBlocks_project_file.Project[0].Build[0].Target;
      var file_units: any[] = result.CodeBlocks_project_file.Project[0].Unit

      var cmake_targets: Map<string, CMakeProject> =
          new Map<string, CMakeProject>();
      ...
      for (var i = 0; i < file_units.length; i++) {
        ...
        if (unit.hasOwnProperty('Option')) {
          for (var j = 0; j < unit.Option.length; j++) {
            if (!unit.Option[j].hasOwnProperty('$') ||
                !unit.Option[j].$.hasOwnProperty('target')) {
              continue;
            }
      ...
      for (var i = 0; i < project_names.length; i++) {
        ...
        var project_binary = new Map<string, string>();
        ...
        project_build.set(
            '', 'cmake --build ' + dir_name + '/BuildFiles --target ' + proj);
        project_clean.set(
            '', 'cmake --build ' + dir_name + '/BuildFiles --target clean');
        project_binary.set('', cmake_proj.Output);
        ...
        var project = new model.Project(
            proj, proj_path, full_name, project_type, project_files,
            project_defines, project_includes, project_flags, project_out_dir,
            project_binary, project_build, project_clean, true, true);
  ...
  public RefreshProject(file: string) {
    var [proj, config]: [model.Project[], string[]] = [[], []];

    if (process.platform === 'win32') {
      [proj, config] = vs.ReloadCmakeProject(file);
    } else {
      [proj, config] = this.ReloadProject(file);
    }
    this.projects_ = {};
    this.configs_ = {};
  ...
  protected GetProjects(file: string): [model.Project[], string[]] {
    if (process.platform === 'win32') {
      return vs.ReloadCmakeProject(file);
    }
